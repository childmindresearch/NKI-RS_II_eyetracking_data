...
for file_path in ds1_present_file_paths:
    with NWBHDF5IO(file_path, 'r') as io:
        ...
        ###############  PUPIL DIAMETER  ###############
        # keys and containers 
        pupil_diameter_key = next(key for key in present_keys if 'Pupil_Diameters_Argus' in key)
...
present_ds1_pupil_diamter = pd.concat(all_dfs_pupil_diameter)
print(present_ds1_pupil_diamter.head())
present_ds1_pupil_diamter.to_csv("/home/nburke/Documents/NKI-RS_II_eyetracking_data_outputs/present_ds1_pupil.csv")
...
for file_path in ds1_sherlock_file_paths:
    with NWBHDF5IO(file_path, 'r') as io:
        ...
        ###############  PUPIL DIAMETER  ###############
        # keys and containers 
        pupil_diameter_key = next(key for key in sherlock_keys if 'Pupil_Diameters_Argus' in key)
        print("pupil diamter key:", pupil_diameter_key)
        # Data Array 
        cst_pupil = nwbfile.acquisition[pupil_diameter_key].data[:]
        # Timestamps for each datapoint
...
sherlock_ds1_pupil_diamter = pd.concat(all_dfs_pupil_diameter)
print(sherlock_ds1_pupil_diamter.head())
sherlock_ds1_pupil_diamter.to_csv("/home/nburke/Documents/NKI-RS_II_eyetracking_data_outputs/sherlock_ds1_pupil.csv")
